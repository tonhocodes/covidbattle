...
function battle(message, user, client, socket, channel) {
            ...
            if (battleStarted) {
                ...
                if (participants.findIndex((i) => i.player === user) === -1) {
                    ...
                    if (passedAlist) {
                        client.action(
                            url,
                            `${user} is a ${aux[1]}! Use !attack (or 1) to attack !alcool (or 2) to defend yourself, !cure (or 3) and !vaccine (or 4) to use the special attack (scientist, doctor), to try to save a infected player!`
                        );
                    }
                }
            }
    ...
    try {
        ...
        if (
            ...
            participant.lastCommand = 0;
            roundAttackers.push(participant.player)
            client.action(url, `${participant.player} You used alcool in gel`);
        }
    } catch (err) {
        console.log(err);
    }
    ...
    if (String(message).split(" ")[0] == "!help") {
        if (String(message).split(" ")[1] == "") {
            client.action(
                url,
                "Available commands: !attack (1),  !alcool (2), !cure (3), !vaccine (4)"
            );
        }
        switch (String(message).split(" ")[1]) {
            ...
            case "vaccine":
                client.action(
                    url,
                    "!vacine | The covid will be attack, taking a variable damage and a bônus damages. Can be used for Doctors and Scientists one time in the match."
                );
                break;
            case "alcool":
                client.action(
                    url,
                    "!alcool | You enter in a defense position, taking less damage from Covid."
                );
                break;
            case "cure":
                client.action(
